...
$sql="update login set password='".$newpass."'where login_id= '".$loginid."'";
    ...
    case 'patientupdate':
    {
        ...
        if(($count8>0)&& (strcmp($oldmobile,$mobile))!=0)
        {
            $json    = '{ "serviceresponse" : { "servicename" : "Update MyPatient Details", "success" : "No", "mobile" : "NULL",  "message" : "Mobile Number Exist" } }';
        }
        else
        {
       ...
       if(($count > 0) && (strcmp($emailold,$email))!=0)
        {
            $json    = '{ "serviceresponse" : { "servicename" : "Update MyPatient Details", "success" : "No", "emaill" : "NULL",  "message" : "Already Exist" } }';
        }
      ...
      $sql = "SELECT * FROM participants_table WHERE participants_id = '".$loginid1."'";
        ...
        if($query11 = mysql_query($sql))
        {
            $userdetail ="update participants_table set fname='".$firstname."',username='".$username."',mobile_num='".$mobile."',gender='".$gender."',city='".$city."',education='".$education."',medical_details='".$medical."',time1='".$timeone."',time1_am_pm='".$timeformat1."',time2='".$timetwo."',Provider_name='".$provider."',time2_am_pm='".$timeformat2."',time3='".$timethree."',time3_am_pm='".$timeformat3."',group_name='".$group."',age='".$age."',email_id='".$email."' WHERE participants_id=".$loginid1;
